...
class Box_Cut(Box):
    """
    CutBox domain: a Box with k fixed linear cut constraints (half-spaces).
    ...
    def submatching(self, other) -> bool:
        """
        Return True if 'other' (zonotope/box) is contained in this Box_Cut.
        We conservatively reduce 'other' to its box hull if needed.
        Conditions:
        ...
        # 1) box-in-box inclusion
        if not ((t_lb <= s_lb).all() and (s_ub <= t_ub).all()):
            return False

        # 2) cut constraints: support of 'other' box in each cut direction
        C_pos = torch.clamp(self.cut_C, min=0.0)
        C_neg = torch.clamp(self.cut_C, max=0.0)
        sup = (C_pos @ s_ub) + (C_neg @ s_lb)  # (k,)

        return bool((sup <= self.cut_c + 1e-9).all())
