...
echo ""
echo "Computing results for Experiment 2 (corresponding to Table 6)"
echo ""

mkdir -p results
#template_domain=box_cut_1
...
do
	...
	do
		...
		do
			method=base
			layer=base
			fn=results/patches_${template_domain}_${dataset}_${net//_best.pth/}_${layer// /+}_${method//_/}_${rep}.txt
			if test -f "$fn"; then
				echo "$fn exists; skipping."
			else
				python . -p --netname ${net} --dataset ${dataset} --num_tests 100 --relu_transformer zonotope --patch_size 2 --template_domain ${template_domain} |& tee "$fn"
			fi
